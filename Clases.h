...
private:
    ...
    std::unordered_map<int, bool> esVisitado; // Mapa sin orden para nodos recorridos
...
public:
    ...
    void DFS(Vertice* verticeInicial) {
        std::stack<Vertice*> pila;
        //std::fill(visited.begin(), visited.end(), false);
        for (auto& par : esVisitado) {
            par.second = false;  // Reinicia el estado de visitado para todos los vértices
        }
        pila.push(verticeInicial);
        std::cout << "Recorrido DFS: ";
        while (!pila.empty()) {
            Vertice* vertice = pila.top();
            pila.pop();
            if (!esVisitado[vertice->id]) {
                std::cout << "[" << vertice->id << "] ";
                esVisitado[vertice->id] = true;
            }

            for (auto i = vertice->adyacencias.begin(); i != vertice->adyacencias.end(); ++i) {
                if (!esVisitado[i->first->id]) {
                    pila.push(i->first);
                }
            }
        }
    ...
    void BFS(Vertice* verticeInicial) {
        ...
        for (auto& par : esVisitado) {
            par.second = false;  // Reinicia el estado de visitado
        }
        esVisitado[verticeInicial->id] = true;
        cola.push(verticeInicial);
        std::cout << "Recorrido BFS";
        while (!cola.empty()) {
            Vertice* vertice= cola.front();
            cola.pop();
            std::cout << "[" << vertice->id << "] ";
            for (auto& adj : vertice->adyacencias) {
                if (!esVisitado[adj.first->id]) {
                    esVisitado[adj.first->id] = true;
                    cola.push(adj.first);
                }
            }
        }
