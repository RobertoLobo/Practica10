...
void MainWindow::on_btnRecorrer_clicked()
{
    adminGrafo->limpiar();
    for (Neurona *n : adminNeuronas->dameNeuronas()){
        adminGrafo->AgregarVertice(n->getId(), n->getX(), n->getY());
    }
    if (!ui->cVerticeInicial->text().isEmpty()){
        adminGrafo->generarAdyacencias();
        adminGrafo->DFS(adminGrafo->dameVertice(ui->cVerticeInicial->text().toInt()));
    }
}


void MainWindow::on_btnRecorrer_2_clicked()
{
    adminGrafo->limpiar();
    for (Neurona *n : adminNeuronas->dameNeuronas()){
        adminGrafo->AgregarVertice(n->getId(), n->getX(), n->getY());
    ...
    if (!ui->cVerticeInicial->text().isEmpty()){
        adminGrafo->generarAdyacencias();
        adminGrafo->BFS(adminGrafo->dameVertice(ui->cVerticeInicial->text().toInt()));
    }
}
